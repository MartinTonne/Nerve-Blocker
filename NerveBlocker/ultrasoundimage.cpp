...
QString UltraSoundImage::getImagePath() const
{
    ...
    return QString("file:" +basePath + "/%1.png").arg(m_imageId);
}

QString UltraSoundImage::getMappedImagePath() const
{
    QString basePath = QStandardPaths::standardLocations(QStandardPaths::DataLocation).value(0);
    return QString("file:" +basePath + "/%1_map.png").arg(m_imageId);
}

QString UltraSoundImage::getMappedImagePathAnswer() const
{
    QString basePath = QStandardPaths::standardLocations(QStandardPaths::DataLocation).value(0);
    return QString(basePath + "/%1_map.png").arg(m_imageId);
}
...
bool UltraSoundImage::hasColorInArea(int x, int y, QRgb color, int radius) const{
    ...
    if(img.isNull()){
         qDebug() << ":"+getMappedImagePathAnswer();
    }
